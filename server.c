...
int check_move(char board[BOARD_SIZE][BOARD_SIZE], int move, int player_id)
{
    if (move == 9 || board[move / BOARD_SIZE][move % BOARD_SIZE] == ' ')
    {
        return 1;
    }
    else
...
void update_board(char board[BOARD_SIZE][BOARD_SIZE], int move, int player_id)
{
    board[move / BOARD_SIZE][move % BOARD_SIZE] = player_id ? 'X' : 'O';
}
...
int check_board(char board[BOARD_SIZE][BOARD_SIZE], int last_move)
{
    int row = last_move / BOARD_SIZE;
    int col = last_move % BOARD_SIZE;
